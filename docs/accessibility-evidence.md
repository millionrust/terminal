...
| `AUD-L10N-LEGACY-001` | A1 | Closed by Goal 21.1.9. The exact whole-UI scan reports zero legacy copy findings; en-US, en-XA, and ar-XB catalogs and typed arguments validate. | 21.1.9 | Resolved; guarded by `scripts/verify/localization.sh --zero-legacy` and `--no-new-baseline`. |
