...
Persisted canvas and agent configuration types belong in `crates/termirust-desktop/src/models.rs` or a small serializable model module re-exported by it, matching the repository's established storage pattern. GPUI entities, focus handles, subscriptions, tasks, channels, and live process handles must never be serialized.
