...
Admission uses separate relay-scoped Ed25519 credentials provisioned through an already authenticated direct route: one Host registration credential and one device route credential. The relay retains the public verifying keys, route ID, revocation epoch and quotas. A single-use challenge binds route, role, epoch, serial, expiry and nonce. Credentials are not G20 long-term keys; private seeds are zeroized and never logged. This spike uses deterministic secrets only in committed synthetic tests and OS CSPRNG generation for non-fixture credentials.
