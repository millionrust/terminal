...
// Source: unicode-width 0.2.2. Do not edit manually.

package com.termirust.mobile.controller

internal object GeneratedTerminalCellWidth {
    const val UNICODE_WIDTH_VERSION = "0.2.2"

    fun width(codePoint: Int): Int = when {
        contains(codePoint, ZERO_WIDTH_RANGES) -> 0
        contains(codePoint, WIDE_RANGES) -> 2
        else -> 1
    }

    private fun contains(value: Int, ranges: IntArray): Boolean {
        var lower = 0
        var upper = ranges.size / 2
        while (lower < upper) {
            val middle = lower + (upper - lower) / 2
            val start = ranges[middle * 2]
            val end = ranges[middle * 2 + 1]
            when {
                value < start -> upper = middle
                value > end -> lower = middle + 1
                else -> return true
            }
        }
        return false
    ...
    private val ZERO_WIDTH_RANGES = intArrayOf(
        0xAD, 0xAD, 0x300, 0x36F, 0x483, 0x489, 0x591, 0x5BD,
        0x5BF, 0x5BF, 0x5C1, 0x5C2, 0x5C4, 0x5C5, 0x5C7, 0x5C7,
        0x605, 0x605, 0x610, 0x61A, 0x61C, 0x61C, 0x64B, 0x65F,
        0x670, 0x670, 0x6D6, 0x6DC, 0x6DF, 0x6E4, 0x6E7, 0x6E8,
        ...
        0xD4D, 0xD4E, 0xD57, 0xD57, 0xD62, 0xD63, 0xD81, 0xD81,
