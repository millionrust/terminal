...
pub struct GenerationArtifacts {
    ...
    /// Colors, spacing, radii, sizes, and type for the iOS app.
    pub swift: String,
    /// The same tokens for the Android app.
    pub kotlin: String,
...
pub fn generate_artifacts(
    ...
    let swift = generate_swift(manifest, &resolved, &source_hash)?;
    let kotlin = generate_kotlin(manifest, &resolved, &source_hash)?;
...
/// The themes the mobile apps resolve against. System follows the device appearance there.
const MOBILE_THEMES: [(Theme, &str, &str); 4] = [
    (Theme::Light, "light", "Light"),
    (Theme::Dark, "dark", "Dark"),
    (Theme::HighContrast, "highContrast", "HighContrast"),
    (Theme::RecordingFriendly, "recording", "Recording"),
];

/// `color.bg.canvas` becomes `colorBgCanvas`.
fn mobile_identifier(key: &str) -> String {
...
fn mobile_token_values(
    token: &TokenDefinition,
    resolved: &BTreeMap<(String, Theme), String>,
) -> Result<Option<Vec<ParsedValue>>, ContractError> {
    if !matches!(
        token.category,
        Category::Color
            | Category::Spacing
    ...
    MOBILE_THEMES
        ...
        .map(|(theme, _, _)| {
            let value = resolved
                .get(&(token.key.clone(), *theme))
                .ok_or_else(|| ContractError::new("internal missing mobile token value"))?;
            parse_typed_value(token.category, value).map_err(|error| {
                ContractError::new(format!("token {} is invalid: {error}", token.key))
            })
        })
        .collect::<Result<Vec<_>, _>>()
        .map(Some)
...
fn mobile_float(value: f32) -> String {
...
enum SlateTokens {\n",
    ...
    let mut first = true;
    for token in &manifest.tokens {
        let Some(values) = mobile_token_values(token, resolved)? else {
            continue;
        };
        if !first {
            output.push('\n');
        ...
        first = false;
        let (return_type, render): (&str, fn(&ParsedValue) -> String) = match token.category {
            Category::Color => ("SlateRGBA", |value| match value {
                ParsedValue::Color(rgba) => format!("SlateRGBA(rgba: 0x{rgba:08X})"),
                _ => String::new(),
            }),
            Category::Typography => ("SlateTypography", |value| match value {
                ParsedValue::Typography(size, line_height, weight) => format!(
                    "SlateTypography(size: {}, lineHeight: {}, weight: {weight})",
                    mobile_float(*size),
                    mobile_float(*line_height)
                ),
                _ => String::new(),
            }),
            _ => ("Double", |value| match value {
                ParsedValue::Float(scalar) => mobile_float(*scalar),
                _ => String::new(),
            }),
        };
        output.push_str(&format!(
        ...
        output.push_str(&format!(
            "    static func {}(_ theme: SlateTheme) -> {return_type} {{\n        switch theme {{\n",
            mobile_identifier(&token.key)
        ));
        for ((_, swift_case, _), value) in MOBILE_THEMES.iter().zip(&values) {
            output.push_str(&format!(
                "        case .{swift_case}: return {}\n",
                render(value)
            ));
        }
        output.push_str("        }\n    }\n");
...
object SlateTokens {\n",
    ...
    for token in &manifest.tokens {
        ...
        first = false;
        let (return_type, render): (&str, fn(&ParsedValue) -> String) = match token.category {
            Category::Color => ("Long", |value| match value {
                ParsedValue::Color(rgba) => {
                    let argb = (rgba >> 8) | ((rgba & 0xff) << 24);
                    format!("0x{argb:08X}L")
                }
                _ => String::new(),
            }),
            Category::Typography => ("SlateTypography", |value| match value {
                ParsedValue::Typography(size, line_height, weight) => format!(
                    "SlateTypography({}f, {}f, {weight})",
                    mobile_float(*size),
                    mobile_float(*line_height)
                ),
                _ => String::new(),
            }),
            _ => ("Float", |value| match value {
                ParsedValue::Float(scalar) => format!("{}f", mobile_float(*scalar)),
                _ => String::new(),
            }),
        };
        let description = format!("{} Allowed use: {}", token.description, token.allowed_use)
            .replace("*/", "* /");
        output.push_str(&format!("    /** {description} */\n"));
        output.push_str(&format!(
            "    fun {}(theme: SlateTheme): {return_type} = when (theme) {{\n",
            mobile_identifier(&token.key)
        ));
        for ((_, _, kotlin_case), value) in MOBILE_THEMES.iter().zip(&values) {
            output.push_str(&format!(
                "        SlateTheme.{kotlin_case} -> {}\n",
                render(value)
            ));
        }
        output.push_str("    }\n");
...
"##;
    ...
    #[test]
    fn mobile_token_files_carry_every_theme_in_platform_color_order() {
        let (manifest, source) = production_manifest();
        let artifacts = generate_artifacts(&manifest, &source).expect("generation should succeed");
        assert_eq!(
            mobile_identifier("color.action.primary_text"),
            "colorActionPrimaryText"
        );
        assert_eq!(mobile_identifier("space.1"), "space1");

        // Swift keeps 0xRRGGBBAA; Kotlin needs 0xAARRGGBB for Compose.
        assert!(
            artifacts
                .swift
                .contains("static func colorBgCanvas(_ theme: SlateTheme) -> SlateRGBA")
        );
        assert!(
            artifacts
                .swift
                .contains("case .dark: return SlateRGBA(rgba: 0x17191DFF)")
        );
        assert!(
            artifacts
                .swift
                .contains("case .light: return SlateRGBA(rgba: 0xFAFBFCFF)")
        );
        assert!(
            artifacts
                .kotlin
                .contains("fun colorBgCanvas(theme: SlateTheme): Long = when (theme) {")
        );
        assert!(artifacts.kotlin.contains("SlateTheme.Dark -> 0xFF17191DL"));
        // A translucent color moves its alpha byte to the front for Kotlin.
        assert!(artifacts.swift.contains("SlateRGBA(rgba: 0x08090B8C)"));
        assert!(artifacts.kotlin.contains("SlateTheme.Dark -> 0x8C08090BL"));
        assert!(
            artifacts
                .swift
                .contains("SlateTypography(size: 13.0, lineHeight: 18.2, weight: 400)")
        );
        assert!(
            artifacts
                .kotlin
                .contains("SlateTypography(13.0f, 18.2f, 400)")
        );
        for ansi in ["Black", "BrightWhite"] {
            assert!(
                artifacts
                    .swift
                    .contains(&format!("colorTerminalAnsi{ansi}"))
            );
            assert!(
                artifacts
                    .kotlin
        ...
        // Durations, shadows, and identifiers stay desktop-only.
        assert!(!artifacts.swift.contains("motionFast"));
        assert!(!artifacts.kotlin.contains("iconStatusIdle"));
        let functions = artifacts.swift.matches("static func ").count();
        assert_eq!(functions, artifacts.kotlin.matches("    fun ").count());
        assert_eq!(
            artifacts.swift.matches("        case .").count(),
            functions * MOBILE_THEMES.len()
        );
    }
