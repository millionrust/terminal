...
usage() {
  cat <<'USAGE'
Usage: scripts/build/mobile-screen-bindings.sh [--clean] [--all|--ios|--android] [--output DIR]

Builds pinned UniFFI 0.32.0 Swift and Kotlin Remote Screens bindings in isolated
temporary target directories. Output is promoted only after every requested
artifact and checksum succeeds.
USAGE
}
...
build_ios() {
  ...
  cp "$GENERATED/${SWIFT_MODULE}FFI.h" "$headers/"
  cp "$GENERATED/${SWIFT_MODULE}FFI.modulemap" "$headers/module.modulemap"
  cp "$GENERATED/${SWIFT_MODULE}.swift" "$STAGED/ios/Sources/"
  ...
  "$ROOT_DIR/scripts/build/ios-static-xcframework.sh" \
    ...
    "$headers" \
    "$STAGED/ios/${SWIFT_MODULE}.xcframework"
}
...
build_android() {
  ...
  mkdir -p "$STAGED/android/kotlin/$KOTLIN_PACKAGE_PATH"
  cp "$GENERATED/$KOTLIN_PACKAGE_PATH/${LIB_STEM}.kt" \
    "$STAGED/android/kotlin/$KOTLIN_PACKAGE_PATH/"
...
mkdir -p "$PARENT_DIR"
NEXT_DIR="$(mktemp -d "$PARENT_DIR/.screens-next.XXXXXX")"
cp -R "$STAGED/." "$NEXT_DIR/"
BACKUP_DIR="$PARENT_DIR/.screens-previous.$$"
...
printf 'Built Remote Screens bindings at %s\n' "$OUTPUT_DIR"
